...
if(isset($_POST) & !empty($_POST)){
	if($_POST['agree'] == true){
		...
		$ivid='trai'.substr(strtoupper(md5(rand())),10,10);
		...
		if($count == 1){
			...
			if($ures){
				...
				$ordsql = "SELECT * FROM trainer WHERE id=$key";
				...
				echo $iosql = "INSERT INTO orders (uid, totalprice, orderstatus, paymentmode, ivid) VALUES ('$uid', '$total', 'Trainer Confirm', '$payment','$ivid')";
				...
				if($iores){
					...
					//query for data that send to email .
				      $information="SELECT name, totalprice, thumb From trainer, orders, orderitems where orders.id=orderitems.orderid and orderitems.pid=trainer.id and orders.ivid='$ivid'";
							...
							echo "there is an error in connection: ".mysql_errno();
			                 ...
			                 //$mail->SMTPDebug = 2;                             
			                  ...
			                  $mail->Body    = '<h4>Hello '.$email.'...</h4><br/><br/><em>
			                     <em>welcome to <a href="http://localhost/upet/">www.upet.com</a>.<br/><h3>
			                      your order is placed:<br><em>your order invoice id is : </em>
			                        '. $ivid . '</h3><br/><br/>Please confirm your payment  for offline payment-method</br><em> </em>';
